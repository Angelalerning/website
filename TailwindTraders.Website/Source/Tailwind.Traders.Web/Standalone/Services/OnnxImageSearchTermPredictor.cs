...
namespace Tailwind.Traders.Web.Standalone.Services {
  public class OnnxImageSearchTermPredictor : IImageSearchTermPredictor {
    ...
    public OnnxImageSearchTermPredictor(
        IWebHostEnvironment environment,
        ILogger<OnnxImageSearchTermPredictor> logger) {
      this.logger = logger;
      var filePath = Path.Combine(environment.ContentRootPath,
                                  "Standalone/OnnxModels/products.onnx");
    ...
    public Task<string> PredictSearchTerm(Stream imageStream) {
      ...
      var prediction = output.First(i => i.Name == "classLabel")
                           .AsEnumerable<string>()
                           .First();
    ...
    private DenseTensor<float> ConvertImageToTensor(Stream imageStream) {
      ...
      image.Mutate(
          ctx => ctx.Resize(new ResizeOptions { Size = new Size(224, 224),
